...
from fullstack_agents.domain.models import LLMRequest, ProductBacklog
from fullstack_agents.domain.ports import StreamingLLM
...
class ProductOwnerAgent:
    ...
    def __init__(self, llm: StreamingLLM, model: str, effort: str | None) -> None:
        ...
        self._model = model
        self._effort = effort
        self.system_prompt = Path(__file__).with_name("system_prompt.md").read_text(encoding="utf-8")
    ...
    def run(self, user_prompt: str, on_delta: Callable[[str], None]) -> tuple[ProductBacklog, str, dict]:
